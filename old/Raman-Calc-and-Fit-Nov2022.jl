...
function ramancalc(pσ, pΣ, BR; df=0.01)
...
setprecision(128)
function passthroughcombined(pσ, pΣin, BR; df=0.01)
    if reflect == true
...
function χ2_oneComp(x,datavel,dataflux)
    ...
    sum(map( x->max(min(x,0.8),-0.8),(pred-dataflux).^2))+(max(24.5,x[4])-24.5)
end
...
MassFromOneComp(x) =  (10.0^x[4])*4*π*(t_eff*3.2e7*500.0*1e5)^2 / (X * Msun * NA)
MassFromTwoComp(x) =  (10.0^x[4]*x[6]+10.0^x[5]*(1-x[6]))*4*π*(t_eff*3.2e7*500.0*1e5)^2 / (X * Msun * NA)
Mass1FromTwoComp(x) = (10.0^x[4]*x[6])*4*π*(t_eff*3.2e7*500.0*1e5)^2 / (X * Msun * NA)
Mass2FromTwoComp(x) = (10.0^x[5]*(1-x[6]))*4*π*(t_eff*3.2e7*500.0*1e5)^2 / (X * Msun * NA)
...
function plot_model_results(pdf)
    ...
    annotate!(-4.5e4, 1.0+0.6*reflect, text("Dust/ISM = $dfrac", :blue, :left, 18))
    plotstyle()
end
...
resbest = TwoCompFitDriver(dataΔv,dataflx; SA=false, degraded = false,
                           fixeddust = true, initguess=[2.0,-0.04,1.5,21.5,22.9,0.5,-6.5])
resbest[2]

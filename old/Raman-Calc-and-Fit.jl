...
data = CSV.read("data/Gemini-etaCarinae-2014-11-fit.csv",DataFrame, header = ["Δv","flux"])
data = CSV.read("data/Magelan-etaCarinae-2015-01-fit.csv",DataFrame, header = ["Δv","flux"])

# We cut out the data near zero velocity.
dataΔv = 1000 .* data.Δv[abs.(data.Δv) .>1]
dataflx = 1.0 .+ data.flux[abs.(data.Δv) .>1]
...
function bestfitWithDegrade(dataΔv,dataflx;degrade=false,SA=true,initguess=[1.995,-0.0347,0.7,0.7,-1.4,22.2,21.0])
    if degrade==true
        dΔv=dataΔv
        dflx=dataflx
    else
...
restable=[bestfitWithDegrade(dataΔv,dataflx;SA=false) for i ∈ 1:100]
χ2 = [restable[i][2] for i ∈ 1:length(restable)]
Mass = [restable[i][4] for i ∈ 1:length(restable)]
dust = [restable[i][3][5] for i ∈ 1:length(restable)]

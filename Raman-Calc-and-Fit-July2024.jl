...
# This reads the cross-section and branching ratio data from the CSV files
begin
    ...
    LyγBRRaw = CSV.read("data/Lyγ-BranchingRatio.csv", DataFrame, header=["Δv0", "br"])
    LyγBr = CubicSpline(LyγBRRaw.Δv0 .* 5 / 1  , LyγBRRaw.br)

    LyγCSRaw = CSV.read("data/Lyγ-TotalCrossSection.csv", DataFrame, header=["Δv0", "cs"])
    LyγCS = CubicSpline(LyγCSRaw.Δv0 .* 5 / 1, LyγCSRaw.cs)
...
# Now plot the cross-sections (fig. 2 in the paper)
begin
    vmin = -40000
    vmax = 40000
    vs=vmin:1000:vmax
    ...
    x_ticks = [T for T ∈ vmin:5000:vmax]
    ...
    rng = vmin:100:vmax
    plot(rng, LyβCS.(rng), label="Total Cross Section", legend=:topleft, ylabel=L"\log_{10}(\sigma_i / cm^2)",
        c=RGB(0.8, 0.3, 0.0), ylims=(-25, -20), xlims=(vmin, vmax), xticks=(x_ticks, x_lbls), xlabel=L"v (km/s)")
    plot!(rng, LyβCS.(rng) .+ log10.(LyβBr.(rng)), label="Partial Cross Section", legend=:topleft,
        c=RGB(0.0, 0.3, 0.8), ylims=(-25, -20), xlims=(vmin, vmax), xticks=(x_ticks, x_lbls),
             yticks=(y_ticks, y_lbls))
    plot!(twinx(), rng, LyβBr.(rng), color=:red, xticks=:none, xlims=(vmin, vmax),
...
#-------------------------------------------------------------------------- 
## Next, we plot the pass through and reflection (that we derived from Mathematica)
#-------------------------------------------------------------------------- 
...
# Now plot the Balmer spectra for various cases
begin
...
# If we want to estimate the errors, we can do a bootstrap analysis. This is the number of bootstrap realizations
n_bootstrap = 20

# To we want to fit pass through or reflection?
reflect = false

# Now we loop over the data files and do the fits
cases = ["Gemini-1", "Magelan-1"]
for data_to_read in cases

    # This table summarizes the fit results. It initializes the table
    if data_to_read == cases[1]
        global results = [[-3, "irrelevant line to ensure results are of type Any"],
    ...
    begin
        if data_to_read == "Gemini-1"
            ...
            data = CSV.read("data/Gemini-etaCarinae-2014-11-short.csv",
                DataFrame, header=["Δv", "flux"])
            global offset = -1
            global data_name = "Gemini 11 2014"
        end
        if data_to_read == "Magelan-1"
            ...
            data = CSV.read("data/Magelan-etaCarinae-2015-01-short.csv",
                DataFrame, header=["Δv", "flux"])
            global offset = 0
            global data_name = "Magellan 01 2015"
        end
        ## We cut out the data near zero velocity, 
        ## and also remove very large velocities (because there are other lines)
        global dataΔv = 0 .+ 1000 .* data.Δv
        global dataflx = data.flux
        global dataΔv_plot = 0 .+ 1000 .* data_plot.Δv
        global dataflx_plot = 1.0 .+ data_plot.flux .+ offset
    ...
    global resbest = OneCompFitDriver(dataΔv, dataflx; SA=false, degraded=false,
    ...
    resbest = TwoCompFitDriver(dataΔv, dataflx; SA=false, degraded=false,
        fixeddust=true, initguess=[2.0, -0.04, 1.5, 22.5, 23.9, 0.5, -6.0, 0.0])
...
# Print the results table here
print(pretty_table(permutedims(hcat(results[3:end]...)); header=results[2], max_num_of_rows=-1) )

# a one-size-fits-all formatter: three significant digits
fmt2sig(x, _row, _col) = x isa AbstractString ? x : @sprintf("%.3g", x)

results_without_χ2 = [results[1],map(r->[r[1:end-2]...,r[end]], results[2:end])...] # remove the χ2 column for the table

print(pretty_table(permutedims(hcat(results_without_χ2[3:end]...)); header=results_without_χ2[2], max_num_of_rows=-1) )

# Latex version for the paper
pretty_table(
    permutedims(hcat(results_without_χ2[3:end]...));
    header     = results_without_χ2[2],
    backend    = Val(:latex),
    formatters = fmt2sig,    # apply to every column
)
